...
#    root_output_dir = os.path.join(cfg.OUTPUT_DIR)

#    final_output_dir = os.path.join(cfg.OUTPUT_DIR, cfg.DATASET.DATASET, cfg.NAME)
...
def summary_model_on_master(model, config, output_dir, copy):
    if comm.is_main_process():
        ...
        shutil.copy2(
            os.path.join(this_dir, '../model', config.MODEL.NAME + '.py'),
            output_dir
        )
...
def resume_checkpoint_multi(model,
                      optimizer,
                      config,
                      output_dir,
                      in_epoch):
    ...
    if config.TRAIN.AUTO_RESUME and os.path.exists(checkpoint):
        ...
        checkpoint_dict = torch.load(checkpoint, map_location='cpu')
        best_perf_od = checkpoint_dict['perf_od']
        best_perf_mol = checkpoint_dict['perf_mol']
        begin_epoch_or_step = checkpoint_dict['epoch' if in_epoch else 'step']
        state_dict = checkpoint_dict['state_dict']
        model.load_state_dict(state_dict)
    ...
    return best_perf_od, best_perf_mol, begin_epoch_or_step

def resume_checkpoint_multi_with_reg(model,
                      optimizer,
                      config,
                      output_dir,
                      in_epoch):
    ...
    if config.TRAIN.AUTO_RESUME and os.path.exists(checkpoint):
        ...
        best_perf_od = checkpoint_dict['perf_od']
        best_perf_mol = checkpoint_dict['perf_mol']
        best_regloss_temp = checkpoint_dict['regloss_temp']
        begin_epoch_or_step = checkpoint_dict['epoch' if in_epoch else 'step']
        state_dict = checkpoint_dict['state_dict']
        model.load_state_dict(state_dict)
    ...
    return best_perf_od, best_perf_mol, best_regloss_temp, begin_epoch_or_step
...
def save_checkpoint_on_master_multi(model,
                              *,
                              distributed,
                              model_name,
                              optimizer,
    ...
    save_dict = {
        ...
        'model': model_name,
        'state_dict': states,
        'perf_od': best_perf_od,
        'perf_mol': best_perf_mol,
        'optimizer': optimizer.state_dict(),
    }
...
def save_checkpoint_on_master_multi_with_reg(model,
                              *,
                              distributed,
                              model_name,
                              optimizer,
    ...
    save_dict = {
        ...
        'perf_od': best_perf_od,
        'perf_mol': best_perf_mol,
        'regloss_temp': best_regloss_temp,
        'optimizer': optimizer.state_dict(),
    }
