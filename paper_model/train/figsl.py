...
def test_multi_with_reg_simpler_test(final_output_dir, val_loader, model, criterion_reg,
    ...
    plt.text(2, 66, s=f"$R^2$={round(r_2, 2)}", fontsize=5)
    plt.text(2, 63, 'RMSE={:.2f}'.format(rmse), fontsize=5)
    plt.text(2, 60, 'MAE={:.2f}'.format(mae), fontsize=5)
    plt.xlim([0, 70.0])
    ...
    plt.savefig(os.path.join(final_output_dir, 'test_Epoch_rmse_liquid.png'), dpi=600)
