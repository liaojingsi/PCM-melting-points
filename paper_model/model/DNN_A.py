...
from paper_model.utils.pointnet2_utils import PointNetSetAbstraction_conv3_relu, PointNetSetAbstraction_relu, \
    square_distance
...
class get_model(nn.Module):
    def __init__(self,num_class_order_disorder, num_class_mol,normal_channel=True):
        ...
        self.sa1 = PointNetSetAbstraction_relu(npoint=500,radius=0.5,nsample=27,in_channel=3+9,
                                                mlp=[32, 32, 32], group_all=False)
        self.sa2_1 = PointNetSetAbstraction_conv3_relu(npoint=500, radius=0.5, nsample=27, in_channel=32 + 3, mlp_kernel=[64, 64, 64], group_all=False)
        self.sa2_2 = PointNetSetAbstraction_conv3_relu(npoint=500, radius=0.5, nsample=27, in_channel=64 + 3,
                                                     mlp_kernel=[256, 256, 256], group_all=False)
        self.sa2_3 = PointNetSetAbstraction_conv3_relu(npoint=500, radius=0.5, nsample=27, in_channel=256 + 3,
                                                     mlp_kernel=[128, 128, 128], group_all=False)
        ...
        self.fcl_res3 = nn.Linear(256, 256)
        self.bnl_res3 = nn.BatchNorm1d(256)
        self.fcl_res4 = nn.Linear(256, 256)
        self.bnl_res4 = nn.BatchNorm1d(256)
        self.fc_lt1 = nn.Linear(256, 128)
        self.bn_lt1 = nn.BatchNorm1d(128)
        self.fc_lt2 = nn.Linear(128, 64)
        self.bn_lt2 = nn.BatchNorm1d(64)
        self.fc_lt3 = nn.Linear(64, 32)
        self.bn_lt3 = nn.BatchNorm1d(32)
        self.fc_lt4 = nn.Linear(32, 1)
        #mbtr
    ...
    def forward(self, input):
        ...
        l2_xyz1, l2_points1 = self.sa2_1(l1_xyz, l1_points)
        l2_xyz2, l2_points2 = self.sa2_2(l2_xyz1, l2_points1)
        l2_xyz, l2_points = self.sa2_3(l2_xyz2, l2_points2)
        ...
        resl = F.relu(self.bnl_res3(self.fcl_res3(resl)))
        resl = F.relu(self.bnl_res4(self.fcl_res4(resl)))

        to_compute_low1 = F.relu(self.bn_lt1(self.fc_lt1(resl)))
        to_compute_low2 = F.relu(self.bn_lt2(self.fc_lt2(to_compute_low1)))
        to_compute_low3 = F.relu(self.bn_lt3(self.fc_lt3(to_compute_low2)))
        to_compute_low = self.fc_lt4(to_compute_low3)
